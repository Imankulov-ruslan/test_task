...
test.describe("Klines API", () => {
  Object.values(KlineIntervals).forEach((interval) => {
    test(`schema for ${interval} interval`, async ({ KlinesService }) => {
      const response = await KlinesService.get({ symbol: Symbols.BTCUSDT, interval });
  ...
  Object.values(KlineIntervals).forEach((interval) => {
    test(`interval value for ${interval} interval`, async ({ KlinesService }) => {
      const response = await KlinesService.get({ symbol: Symbols.BTCETH, interval });
      const { body } = response;
      for (const item of body.items) {
  ...
  test("invalid symbol", async ({ KlinesService }) => {
    const response = await KlinesService.get({ symbol: "INVALID", interval: KlineIntervals["1s"] });
    const { body, statusCode } = response;
    expect(statusCode).toBe(200);
  ...
  test("invalid interval", async ({ KlinesService }) => {
    const response = await KlinesService.get({ symbol: Symbols.BTCUSDT, interval: "INVALID" as KlineIntervals });
    const { body, statusCode } = response;
  ...
  for(const limit of [0, 1, 1000]) {
    test(`valid limit ${limit}`, async ({ KlinesService }) => {
      const response = await KlinesService.get({ symbol: Symbols.BTCUSDT, interval: KlineIntervals["1s"]}, {limit});
      const { body, statusCode } = response;
      expect(statusCode).toBe(200);
  ...
  for(const invalidLimit of [-1, 10_001]) {
    test(`invalid limit ${invalidLimit}`, async ({ KlinesService }) => {
      const response = await KlinesService.get({ symbol: Symbols.BTCUSDT, interval: KlineIntervals["1s"]}, {limit: invalidLimit });
      const { body, statusCode } = response;
      expect(statusCode).toBe(400);
      expect(body.message).toBe("Wrong query params");
    });
  ...
  const testData = {
    ...
    test(`${key}`, async ({ KlinesService }) => {
      const response = await KlinesService.get({ symbol: Symbols.BTCUSDT, interval: KlineIntervals["1s"]}, { [key]: testData[key] });
      const { body, statusCode } = response;
      expect(statusCode).toBe(200);
      for(const item of body.items) {
