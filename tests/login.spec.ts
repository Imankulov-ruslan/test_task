...
test.describe('Auth user', () => {
  ...
  test('correct login and password', async ({ AuthServiceWithoutCookies }) => {
    const reponse = await AuthServiceWithoutCookies.login<{id:string, email:string, role:string}>(validUsername, validPassword)
    ...
    test('incorrect login', async ({ AuthServiceWithoutCookies }) => {
      const reponse = await AuthServiceWithoutCookies.login<{message:string}>(validUsername, 'incorrectPassword')
      const {body, statusCode, headers} = reponse
    ...
    test('incorrect login as number', async ({ AuthServiceWithoutCookies }) => {
      const reponse = await AuthServiceWithoutCookies.login<{message:string}, string, number>(validUsername, 123)
      const {body, statusCode, headers} = reponse
      expect(statusCode).toBe(400)
      expect(body.message).toEqual('Wrong login or password')
      expect(headers['set-cookie']).not.toBeDefined()
    ...
    test('short password', async ({ AuthServiceWithoutCookies }) => {
      const reponse = await AuthServiceWithoutCookies.login<{message:string}>(validUsername, '')
      const {body, statusCode, headers} = reponse
      expect(statusCode).toBe(400)
    ...
    test('long password', async ({ AuthServiceWithoutCookies }) => {
      const reponse = await AuthServiceWithoutCookies.login<{message:string}>(validUsername, faker.string.sample(51))
      const {body, statusCode, headers} = reponse
      expect(statusCode).toBe(400)
      expect(body.message).toEqual('"password" must be between 1 and 50 characters')
      expect(headers['set-cookie']).not.toBeDefined()
